...
static int bitrate_to_mode(float bitrate) {
	if(bitrate <= 6.6f)
		return AMR_WB_MODE_660;
	if(bitrate <= 8.85f)
		return AMR_WB_MODE_885;
	if(bitrate <= 12.65f)
		return AMR_WB_MODE_1265;
	if(bitrate <= 14.25f)
		return AMR_WB_MODE_1425;
	if(bitrate <= 15.85f)
		return AMR_WB_MODE_1585;
	if(bitrate <= 18.25f)
		return AMR_WB_MODE_1825;
	if(bitrate <= 19.85f)
		return AMR_WB_MODE_1985;
	if(bitrate <= 23.05f)
		return AMR_WB_MODE_2305;
...
static int amr_wb_encoder_init(
...
#ifdef HAVE_AMR_WB_ENCODE
static int amr_wb_encode_frame(struct mux_encoder *enc, struct amr_wb_encoder_data *data) {
	uint8_t frame_buf[AMR_WB_MAX_FRAME_SIZE];
	int frame_size = E_IF_encode(data->encoder, data->mode, data->input_buf, frame_buf, data->dtx);
	if(frame_size < 0) {
		mux_encoder_set_error(
			enc,
			MUX_ERROR_ENCODE,
...
static int amr_wb_encoder_encode(
	struct mux_encoder *enc,
	const void *input,
	size_t input_size,
	int stream_type
	...
	while(avail > 0) {
		...
		memcpy(&data->input_buf[data->input_samples], samples, copy * sizeof(int16_t));
...
static int amr_wb_encoder_finalize(struct mux_encoder *enc) {
	...
	if(data->input_samples > 0) {
		memset(
			&data->input_buf[data->input_samples],
			0,
...
static int
amr_wb_decoder_init(struct mux_decoder *dec, const struct mux_param *params, int num_params) {
...
static int
amr_wb_decode_one(struct mux_decoder *dec, struct amr_wb_decoder_data *data, uint8_t *amr_frame) {
...
/* Parser emit shim: AMR-WB frames are self-delimiting (mode byte -> length),
 * so split the raw audio byte stream by mode byte for both mux and passthrough.
 * Side channel passes through. */
static int amr_wb_parser_emit(void *user, int stream_type, const void *chunk, size_t size) {
...
static int amr_wb_decoder_decode(struct mux_decoder *dec, const void *input, size_t input_size) {
	struct amr_wb_decoder_data *data;
