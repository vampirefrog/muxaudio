...
static int write_opus_header(ogg_stream_state *os, int sample_rate, int channels, int pre_skip) {
...
static int mux_opus_encoder_init(
	...
	data->enc = opus_encoder_create(sample_rate, num_channels, OPUS_APPLICATION_AUDIO, &opus_error);
...
static int opus_encode_frame(
	...
	len = opus_encode(data->enc, frame_pcm, data->frame_size, packet, sizeof(packet));
	if(len < 0) {
		mux_encoder_set_error(
			enc,
			MUX_ERROR_ENCODE,
...
static int mux_opus_encoder_encode(
	struct mux_encoder *enc,
	const void *input,
	size_t input_size,
	int stream_type
	...
	if(data->pending_samples > 0) {
		...
		memcpy(data->pending + data->pending_samples * nch, pcm, take * nch * sizeof(int16_t));
	...
	rem = in_samples - idx;
	if(rem > 0) {
		memcpy(data->pending, pcm + idx * nch, rem * nch * sizeof(int16_t));
...
static int mux_opus_encoder_finalize(struct mux_encoder *enc) {
	...
	if(data->pending_samples > 0) {
		...
		memset(
			data->pending + data->pending_samples * nch,
			0,
			((size_t)data->frame_size - data->pending_samples) * nch * sizeof(int16_t)
		);
		ret = opus_encode_frame(enc, data, data->pending);
		if(ret)
			return ret;
...
static int
mux_opus_decoder_init(struct mux_decoder *dec, const struct mux_param *params, int num_params) {
...
static int
parse_opus_header(const unsigned char *packet, int bytes, int *sample_rate, int *channels) {
...
static int mux_opus_decoder_decode(struct mux_decoder *dec, const void *input, size_t input_size) {
	struct opus_decoder_data *data;
	char *buffer;
	ogg_page og;
	ogg_packet op;
	...
	if(!buffer) {
		mux_decoder_set_error(dec, MUX_ERROR_NOMEM, "ogg_sync_buffer failed", "libogg", 0, NULL);
		return MUX_ERROR_NOMEM;
	}
		...
		while(ogg_stream_packetout(&data->os_audio, &op) == 1) {
			if(!data->decoder_inited) {
				...
				if(data->sample_rate > 0 && data->num_channels > 0) {
					int opus_error;
					data->dec =
						opus_decoder_create(data->sample_rate, data->num_channels, &opus_error);
			...
			if(data->decoder_inited && op.bytes > 0) {
				...
				samples = opus_decode(data->dec, op.packet, op.bytes, pcm_buf, 5760, 0);
				if(samples < 0) {
					mux_decoder_set_error(
						dec,
						MUX_ERROR_DECODE,
				...
				if(samples > 0) {
					size_t sz = (size_t)samples * data->num_channels * sizeof(int16_t);
					ret = mux_decoder_emit(dec, MUX_STREAM_AUDIO, pcm_buf, sz);
					if(ret)
						return ret;
				}
			}
	...
	if(data->have_side_stream) {
		while(ogg_stream_packetout(&data->os_side, &op) == 1) {
			...
			if(op.e_o_s && op.bytes == 0)
				continue;
			ret = mux_decoder_emit(dec, MUX_STREAM_SIDE_CHANNEL, op.packet, op.bytes);
			if(ret)
				return ret;
		}
	}
