...
static int amr_encode_frame(struct mux_encoder *enc, struct amr_encoder_data *data) {
	...
	int frame_size =
		Encoder_Interface_Encode(data->encoder, data->mode, data->input_buf, frame_buf, 0);
	if(frame_size < 0) {
		mux_encoder_set_error(
			enc,
			MUX_ERROR_ENCODE,
...
static int
amr_encoder_encode(struct mux_encoder *enc, const void *input, size_t input_size, int stream_type) {
	struct amr_encoder_data *data;
...
static int
amr_decoder_init(struct mux_decoder *dec, const struct mux_param *params, int num_params) {
...
static int
amr_decode_one(struct mux_decoder *dec, struct amr_decoder_data *data, uint8_t *amr_frame) {
...
static int amr_parser_emit(void *user, int stream_type, const void *chunk, size_t size) {
...
static int amr_decoder_decode(struct mux_decoder *dec, const void *input, size_t input_size) {
	struct amr_decoder_data *data;
...
static const char *const amr_file_extensions[] = { "amr" };
...
const struct mux_codec_ops mux_codec_amr_ops = {
	...
	.file_extensions = amr_file_extensions,
	.file_extension_count = sizeof(amr_file_extensions) / sizeof(amr_file_extensions[0])
};
