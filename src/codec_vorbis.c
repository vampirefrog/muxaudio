...
static int vorbis_encoder_init(
	...
	if(bitrate > 0)
		ret = vorbis_encode_init(&data->vi, num_channels, sample_rate, -1, bitrate * 1000, -1);
	else
		ret = vorbis_encode_init_vbr(&data->vi, num_channels, sample_rate, quality);
...
static int vorbis_encoder_encode(
	struct mux_encoder *enc,
	const void *input,
	size_t input_size,
	int stream_type
...
static int
vorbis_decoder_init(struct mux_decoder *dec, const struct mux_param *params, int num_params) {
...
static int vorbis_emit_pcm(struct mux_decoder *dec, float **pcm, int samples, int channels) {
	int done = 0;
	...
	while(done < samples) {
		...
		ret = mux_decoder_emit(dec, MUX_STREAM_AUDIO, tmp, (size_t)m * channels * sizeof(int16_t));
		if(ret)
			return ret;
...
static int vorbis_decoder_decode(struct mux_decoder *dec, const void *input, size_t input_size) {
	struct vorbis_decoder_data *data;
	char *buffer;
	ogg_page og;
	ogg_packet op;
			...
			if(data->decoder_inited) {
				if(vorbis_synthesis(&data->vb, &op) == 0) {
					...
					while((samples = vorbis_synthesis_pcmout(&data->vd, &pcm)) > 0) {
						ret = vorbis_emit_pcm(dec, pcm, samples, data->vi.channels);
						if(ret)
							return ret;
