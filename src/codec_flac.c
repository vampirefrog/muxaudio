...
static int mux_flac_encoder_init(
	...
	if(num_channels < 1 || num_channels > 8) {
		mux_encoder_set_error(enc, MUX_ERROR_INVAL, "FLAC supports 1-8 channels", NULL, 0, NULL);
	...
	init_status =
		FLAC__stream_encoder_init_stream(data->enc, flac_write_callback, NULL, NULL, NULL, data);
...
static int mux_flac_encoder_encode(
	struct mux_encoder *enc,
	const void *input,
	size_t input_size,
	int stream_type
	...
	for(off = 0; off < total;) {
		...
		for(i = 0; i < blk; i++)
			for(ch = 0; ch < nch; ch++)
				data->planar[ch][i] = pcm[(off + i) * nch + ch];

		if(!FLAC__stream_encoder_process(
			   data->enc,
			   (const FLAC__int32 *const *)data->planar,
			   blk
		   )) {
			FLAC__StreamEncoderState st = FLAC__stream_encoder_get_state(data->enc);
			mux_encoder_set_error(
				enc,
				MUX_ERROR_ENCODE,
...
static FLAC__StreamDecoderWriteStatus flac_dec_write_callback(
	...
	while(i < bs) {
		...
		r = mux_decoder_emit(dec, MUX_STREAM_AUDIO, tmp, (size_t)m * nch * sizeof(int16_t));
		if(r) {
			data->emit_ret = r;
...
static void flac_dec_metadata_callback(
	const FLAC__StreamDecoder *decoder,
...
static void flac_dec_error_callback(
	const FLAC__StreamDecoder *decoder,
...
static int
mux_flac_decoder_init(struct mux_decoder *dec, const struct mux_param *params, int num_params) {
...
static int flac_feed_audio(struct flac_decoder_data *d, const uint8_t *in, size_t n) {
	if(!d->buf) {
...
static int flac_parser_emit(void *user, int stream_type, const void *chunk, size_t size) {
...
static int mux_flac_decoder_decode(struct mux_decoder *dec, const void *input, size_t input_size) {
	struct flac_decoder_data *data;
