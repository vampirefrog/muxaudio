 ...
 */
static void mulaw_encoder_deinit(struct mux_encoder *enc) { (void)enc; }

/*
 * Mu-law encoder encode
 * Converts 16-bit PCM samples to 8-bit mu-law
 */
static int mulaw_encoder_encode(
	struct mux_encoder *enc,
	const void *input,
	size_t input_size,
	int stream_type
 ...
 */
static int
mulaw_decoder_init(struct mux_decoder *dec, const struct mux_param *params, int num_params) {
 ...
 */
static int mulaw_parser_emit(void *user, int stream_type, const void *data, size_t size) {
 ...
 */
static int mulaw_decoder_decode(struct mux_decoder *dec, const void *input, size_t input_size) {
	struct mulaw_decoder_data *data;
