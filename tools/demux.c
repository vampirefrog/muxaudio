...
static void usage(const char *prog) {
	...
	fprintf(
		stderr,
		"  -s, --streams NUM      Number of streams: 1=passthrough, 2=mux (default: 2)\n"
	);
...
static int decode_stream(const struct decoder_config *config) {
	...
	dec = mux_decoder_new(config->codec, config->num_streams, NULL, 0, emit_cb, &stats);
	if(!dec) {
	...
	if(config->verbose)
		fprintf(
			stderr,
			"Decoded: %zu bytes audio, %zu bytes side channel\n",
			stats.total_audio,
			stats.total_side
		);
...
int main(int argc, char **argv) {
	struct decoder_config config = {.codec = MUX_CODEC_FLAC, .num_streams = 2, .verbose = 0};
