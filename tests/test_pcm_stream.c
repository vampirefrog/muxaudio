...
static int emit_cb(void *user, int stream_type, const void *data, size_t size) {
	...
	buf_append(stream_type == MUX_STREAM_AUDIO ? &c->audio : &c->side, data, size);
	return 0;
}
...
static int encode_fixture(enum mux_codec_type codec, int rate, struct buf *out) {
	struct mux_encoder *enc = mux_encoder_new(codec, rate, 1, 2, NULL, 0, sink_cb, out);
	if(!enc) {
		printf("  encoder_new failed\n");
		return -1;
	}
...
static int
decode_chunked(enum mux_codec_type codec, struct buf *enc, size_t chunk, struct collector *c) {
	memset(c, 0, sizeof(*c));

	struct mux_decoder *dec = mux_decoder_new(codec, 2, NULL, 0, emit_cb, c);
	if(!dec) {
		printf("  decoder_new failed\n");
		return -1;
	}
...
static void compare(struct collector *ref, struct collector *got, const char *label) {
	CHECK(buf_eq(&ref->audio, &got->audio), "audio matches reference");
	CHECK(buf_eq(&ref->side, &got->side), "side channel matches reference");
	(void)label;
}

static void run_codec(enum mux_codec_type codec, const char *name, int lossless, int rate) {
	printf("== %s ==\n", name);
	...
	for(size_t k = 0; k < sizeof(chunks) / sizeof(chunks[0]); k++) {
		...
		CHECK(decode_chunked(codec, &enc, chunks[k], &got) == 0, "chunked decode rc");
...
int main(void) {
	for(int i = 0; i < 512; i++)
		audio1[i] = (int16_t)(i * 37 - 9000);
	for(int i = 0; i < 300; i++)
		audio2[i] = (int16_t)(1000 - i * 11);
