...
int main(void)
{
	...
	if (th_encode(MUX_CODEC_MP3, SAMPLE_RATE, NUM_CHANNELS, 2, params, 3,
		      pcm, sizeof(pcm), side_data, sizeof(side_data),
		      &muxed) != MUX_OK) {
	...
	else { fprintf(stderr, "\xe2\x9c\x97 No audio decoded\n"); rc = 1; }
	...
	else { fprintf(stderr, "\xe2\x9c\x97 Side channel mismatch\n"); rc = 1; }
