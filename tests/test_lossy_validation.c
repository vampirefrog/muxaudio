			 ...
			 float min_snr_db, int max_offset)
{
	...
	enc = mux_encoder_new(MUX_CODEC_MP3, SAMPLE_RATE, NUM_CHANNELS,
			      params, 2);
	if (!enc) {
...
int main(void)
{
	...
	/*
	 * Test 1: Sine wave (simple tone, MP3 should handle well)
	 */
	...
	/*
	 * Test 2: Sinc pulse (band-limited impulse, challenging for MP3)
	 */
	...
	/*
	 * Test 4: Chirp (frequency sweep, very challenging)
	 */
	...
	if (temp) {
		...
		if (test_waveform("C Major Chord (C-E-G)", test_signal, 0.0f, 1024) != 0)
			failed++;
	}
