...
static int
decode_chunked(int streams, const struct th_buf *enc, size_t chunk, struct collector *out) {
...
static void run(int streams, const int16_t *pcm) {
	...
	if(th_encode(
		   ...
		   1,
		   pcm,
		   (size_t)NSAMP * sizeof(int16_t),
		   have_side ? side1 : NULL,
		   have_side ? strlen(side1) : 0,
		   &enc
	   ) != MUX_OK) {
	...
	printf(
		"  encoded %d samples -> %zu bytes (~%d frames)\n",
		NSAMP,
		enc.len,
		(FRAMES + 4095) / 4096
	);
	...
	for(size_t k = 0; k < sizeof(chunks) / sizeof(chunks[0]); k++) {
		...
		CK(c.audio.len == (size_t)NSAMP * sizeof(int16_t), "decoded byte count == source");
		...
		if(have_side)
			CK(c.side.len == strlen(side1) && memcmp(c.side.data, side1, strlen(side1)) == 0,
			   "side channel intact");
		th_buf_free(&c.audio);
		th_buf_free(&c.side);
	}
...
int main(void) {
	...
	for(i = 0; i < FRAMES; i++) {
		...
		int16_t s = (int16_t)(6000.0 * __builtin_sin(2.0 * 3.14159265 * 330.0 * i / RATE)) + noise;
		pcm[i * CH] = s;
