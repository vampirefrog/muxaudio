...
static int roundtrip_exact(const char *name, int channels, int level, const int16_t *pcm) {
	...
	if(out.audio.len != bytes || memcmp(out.audio.data, pcm, bytes) != 0) {
...
int main(void) {
	...
	if(roundtrip_exact("Sine mono", 1, 5, sig) != 0)
		failed++;

	generate_sine(sig, NUM_SAMPLES, 2, SAMPLE_RATE, 440.0f, 0.6f);
	if(roundtrip_exact("Sine stereo", 2, 8, sig) != 0)
		failed++;

	generate_square(sig, NUM_SAMPLES, 2, SAMPLE_RATE, 220.0f, 0.5f);
	if(roundtrip_exact("Square stereo", 2, 5, sig) != 0)
		failed++;

	generate_noise(sig, NUM_SAMPLES, 2, 0.8f);
	if(roundtrip_exact("Noise stereo", 2, 0, sig) != 0)
		failed++;
