...
void generate_sine(
	...
	for(i = 0; i < num_samples; i++) {
		...
		int16_t sample = (int16_t)(sin(2.0 * M_PI * frequency * t) * 32767.0 * amplitude);
...
void generate_square(
	...
	for(i = 0; i < num_samples; i++) {
		...
		int16_t sample = (int16_t)((phase < 0.5f ? 1.0f : -1.0f) * 32767.0 * amplitude);
		for(ch = 0; ch < (size_t)channels; ch++)
			buffer[i * channels + ch] = sample;
	}
}
...
void generate_noise(int16_t *buffer, size_t num_samples, int channels, float amplitude) {
	size_t i, ch;
	for(i = 0; i < num_samples; i++) {
		for(ch = 0; ch < (size_t)channels; ch++) {
...
void generate_chirp(
	...
	for(i = 0; i < num_samples; i++) {
		...
		float phase =
			2.0f * M_PI * (start_freq * t + 0.5f * (end_freq - start_freq) * t * t / duration);
...
float calculate_snr(const int16_t *original, const int16_t *decoded, size_t num_samples) {
...
float calculate_rmse(const int16_t *original, const int16_t *decoded, size_t num_samples) {
...
int find_time_offset(
	...
	for(offset = -max_offset; offset <= max_offset; offset++) {
		...
		for(i = 0; i < search_len; i++) {
			...
			if(orig_idx >= 0 && orig_idx < (int)original_samples && dec_idx >= 0 &&
			   dec_idx < (int)decoded_samples) {
				correlation += (double)original[orig_idx] * (double)decoded[dec_idx];
				count++;
			}
		}
...
int validate_lossy_audio(
	...
	printf(
		"  Time offset: %d samples (%.2f ms @ 44.1kHz)\n",
		offset_samples,
		offset_samples * 1000.0f / 44100.0f
	);
...
void print_audio_stats(
	...
	/* Find offset */
	offset = find_time_offset(original, original_samples, decoded, decoded_samples, 2048);
