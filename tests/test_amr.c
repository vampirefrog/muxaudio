...
static int test_one(enum mux_codec_type codec, const char *name, int rate) {
	...
	rc = th_encode(codec, rate, 1, 1, NULL, 0, pcm, nframes * sizeof(int16_t), NULL, 0, &muxed);
	if(rc != MUX_OK) {
...
int main(void) {
	...
	if(test_one(MUX_CODEC_AMR, "AMR-NB", 8000) == 1)
		failures++;
	if(test_one(MUX_CODEC_AMR_WB, "AMR-WB", 16000) == 1)
		failures++;
