...
static inline int th_encode(
	...
	e = mux_encoder_new(codec, rate, channels, num_streams, params, num_params, th_sink, out);
	if(!e)
		return MUX_ERROR;
...
static inline int th_emit(void *user, int stream_type, const void *data, size_t size) {
	struct th_out *o = user;
	return th_append(stream_type == MUX_STREAM_AUDIO ? &o->audio : &o->side, data, size);
}

/* Decode a whole muxed blob, collecting audio and side output. */
static inline int th_decode(
	enum mux_codec_type codec,
	int num_streams,
	const uint8_t *muxed,
	size_t muxed_len,
...
static inline void
th_sine(int16_t *buf, int nframes, int channels, int rate, double freq, double amp) {
	int i, ch;
	for(i = 0; i < nframes; i++) {
		double v = amp * 32767.0 * __builtin_sin(2.0 * 3.14159265358979 * freq * i / rate);
